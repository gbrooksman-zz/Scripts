...
select 'CREATE TABLE TMP_DATACODES (CODE NVARCHAR2(4000));';
SELECT DISTINCT 'INSERT INTO TMP_DATACODES (CODE) VALUES ('''+D.F_DATA_CODE+''');'
FROM T_MSDSTYPE_SECTIONS M
JOIN T_DATATYPES D ON D.F_SECTION = M.F_SECTION AND D.F_FORMAT = M.F_FORMAT
JOIN T_GCS_RESERVED_DATA_CODES G ON G.F_DATA_CODE = D.F_DATA_CODE
WHERE M.F_FORMAT = @@Format@@
...
select 'CREATE TABLE TMP_TEXTCODES(D NVARCHAR2(4000), P NVARCHAR2(4000), T NVARCHAR2(4000));';
SELECT DISTINCT 'INSERT INTO TMP_TEXTCODES (D, P, T) VALUES ('''+L.F_DATA_CODE+''', '''+REPLACE(T.F_PHRASE,CHAR(39),CHAR(39)+CHAR(39))+''' , '''+L.F_TEXT_CODE+''');'
FROM T_MSDSTYPE_SECTIONS M
JOIN T_DATATYPES D ON D.F_SECTION = M.F_SECTION AND D.F_FORMAT = M.F_FORMAT
JOIN T_GCS_RESERVED_DATA_CODES G ON G.F_DATA_CODE = D.F_DATA_CODE
JOIN T_PHRASE_LINKAGE L ON L.F_DATA_CODE = G.F_DATA_CODE
